...
if(isfreelancer()){
    ...
    if(isset($_POST['addJobFreelancer'])){
        ...
        $freelancerId = $_SESSION['id'];
        $regions = $_POST['location'];
        $editJobQuery = $pdo->prepare("UPDATE jobs SET job_title=:job_title,job_description=:job_description,job_price=:job_price,category_id=:cat WHERE job_id=:id AND user_id=:uid");
